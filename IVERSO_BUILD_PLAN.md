...
│    • Hero: isMobile ? Canvas2D : Three.js       │
...
| Elem | Gép | Mobil |
|------|-----|-------|
| Hero technológia | Three.js (3D particle rendszer) | CSS + Canvas 2D |
| Hero parázs | Hover erősödik + kattintás robban | Touch/tap robban |
| Hero részecskék | Több, nagyobb, 3D mélység | Kevesebb, kisebb, 2D |
...
- [ ] Three.js 3D parázs particle rendszer (desktop)
- [ ] Canvas 2D parázs fallback (mobil)
- [ ] Automatikus váltás useIsMobile() alapján
- [ ] IVERSO felirat + "Let's build something" fade-in
- [ ] Interakciók: hover erősödés (desktop), kattintás/tap robbanás (mindkettő)
- [ ] Mobil optimalizáció (kevesebb particle, touch)
- [ ] Scroll-ra parázs halványul (2. jelenet átmenet)

**Eredmény:** Megnyitod az oldalt → parázs hero él, reagál. Gépen 3D, mobilon 2D.
...
| Feladat | Hol csináld |
|---------|------------|
| Hero Three.js 3D fejlesztés | VS Code (gépen kell látni) |
| Hero Canvas 2D mobil fallback | VS Code (gépen kell látni) |
| Morph animáció (demó → aloldal) | VS Code (vizuális) |
| Drag & drop építős | VS Code (vizuális) |
...
| Demó előzetesek struktúra | Claude Code telefonról OK (animáció polish gépen) |
